...
namespace Bionuclear.Infrastructure
{
    public class FileShare : IFileShare
    {
        ...
        public async Task FileUploadAsync(FilesDetails fileDetails)
        {
            // Get the configurations and create share object
            ShareClient share = new ShareClient(_config.GetConnectionString("STORAGE_AZURE_FILES"), _config.GetValue<string>("FileShareDetails:FileShareName"));
            ...
            if (await share.ExistsAsync())
            {
                ...
                if (await directory.ExistsAsync())
                {
                    ...
                    {
                        await fileDetails.FileDetail.CopyToAsync(stream);
                    }
        ...
        public async Task<string> FileDownloadAsync(string fileShareName)
        {
            ShareClient share = new ShareClient(_config.GetConnectionString("STORAGE_AZURE_FILES"), _config.GetValue<string>("FileShareDetails:FileShareName"));
            ShareDirectoryClient directory = share.GetDirectoryClient("FileShareDemoFiles");
